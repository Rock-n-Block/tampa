  ...
  ({
    ...
    const onWithdrawClick = async (stake, index, stakeId, end) => {
      ...
      const seconds = moment
        .utc(stake.start * 1000)
        .diff(moment.utc(+currentDayUnix * 1000), 'seconds');
    ...
    return (
      <>
        <div className="container stakes" id="stakes">
          <div
            className={classNames('stakes__refresh', {
              refreshing: isRefreshingStates,
            })}
            onClick={handleRefresh}>
            {isDarkTheme ? (
              <img src={refreshDarkImg} alt="" />
            ) : (
            ...
            <div className="stakes__nav">
              {navItems.map((item, index) => {
                return (
                  <div
                    key={index}
                    onClick={() => handlecChangeNav(index)}
                    className={classNames('stakes__nav-item', {
          ...
          <div
            ...
            <div className="stakes__row-head-item">
              {!activeTab ? 'Current Value' : 'paid amount'}
            </div>
          </div>
                  ...
                  return (
                    <div
                      ...
                      className={classNames(
                        'container stakes__row t-row t-row__content',
                        {
                          'stakes__row--ended': item.isEnded,
                        },
                      )}>
                      <div className="stakes__row-item" data-name="Start">
                        {dateFormat(+item.start)}
                      </div>
                      <div className="stakes__row-item" data-name="End">
                        {dateFormat(+item.end)}
                      </div>
                      {!item.isEnded && (
                      ...
                      <div className="stakes__row-item" data-name="Staked">
                        <RowItemTooltip
                          tooltipText={formatNumberWithCommas(item.staked)}
                          parent="stakes">
                      ...
                      <div className="stakes__row-item" data-name="Shares">
                        <RowItemTooltip
                          tooltipText={formatNumberWithCommas(item.shares)}
                          parent="stakes">
                          {formatNumberWithCommas(item.shares)}
                        </RowItemTooltip>
                      </div>
                      <div
                        className="stakes__row-item"
                        data-name="BonusDay Rewards">
                        <RowItemTooltip
                          tooltipText={item.bonusday}
                          parent="stakes">
                          {item.bonusday}
                        </RowItemTooltip>
                      </div>
                      <div
                        className="stakes__row-item"
                        data-name="eth Dividends Rewards">
                        <RowItemTooltip
                          tooltipText={item.dividents}
                          parent="stakes">
                          {item.dividents}
                        </RowItemTooltip>
                      </div>
                      <div
                        className="stakes__row-item stakes__red"
                        data-name="Interest">
                        <RowItemTooltip
                          tooltipText={formatNumberWithCommas(item.interest)}
                          parent="stakes">
                      ...
                      {!item.isEnded && (
                        <button
                          ...
                          className="stakes__btn btn btn--withdraw">
                          withdraw
                        </button>
                      )}
                    </div>
        ...
        <Modal
          ...
          <div className="stakes__modal">
            ...
            <div className="stakes__modal-btns">
              <button
                className="stakes__modal-btn stakes__modal-btn--cancel btn btn--withdraw"
                onClick={() => setVisibleModal(false)}>
                CLOSE
              </button>
              <button
                className="stakes__modal-btn btn btn--withdraw"
                onClick={() => {
                  handleWithdraw(activeStake.index, activeStake.stakeId);
