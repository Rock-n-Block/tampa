      ...
      <div className="container s-graph" id="s-graph">
        <div className="s-graph__head">
          <div className="s-graph__wrapper">
            <div className="s-graph__text">MATIC DIVIDENDS HISTORY</div>
            {to && (
              <Link to={to}>
                <button className="s-graph__btn btn btn--md">
...
More details will be provided as the project progresses so make sure to check for new info because there will also be times this 2.5% is allowed to accumulate for days or weeks in order to have Maga Jackpots. Keep in mind, the lottery is based off of yesterday's pool. So the larger the pool from the day before, the higher amount of MATIC will be won the next day by a winner who is randomly chosen by the smart contract.
        ...
        <div className="s-graph__content">
          {!data.length && (
            <div className="s-graph__loader">
              {isDarkTheme ? (
                <img
                  src={refreshDarkImg}
                  alt=""
                  className="s-graph__loader-img"
                />
              ) : (
                <img src={refreshImg} alt="" className="s-graph__loader-img" />
              )}
            </div>
          )}
          {
            <div
              ...
              <ResponsiveContainer width="100%" height={400}>
                <LineChart data={data} syncId="test">
                  <CartesianGrid
                    stroke={isDarkTheme ? '#252253' : '#FCFCFF'}
                    fill={isDarkTheme ? '#363362' : '#F8F7FD'}
                    strokeWidth="10"
                  />
                  <XAxis
                    // type="number"
                    ...
                    stroke={isDarkTheme ? '#53B9EA' : '#DB4848'}></XAxis>
                  <Tooltip
                    content={({ payload }) => (
                      <div className="s-graph__tooltip">
                        <div className="s-graph__tooltip-circle"></div>
                        <div className="s-graph__tooltip-value">
                  ...
                  <YAxis
                    ...
                    dataKey="value"
                    tickLine={false}
                    stroke={isDarkTheme ? '#53B9EA' : '#DB4848'}></YAxis>
                  <Line
                    key="value"
                    type="monotone"
                    dataKey="value"
