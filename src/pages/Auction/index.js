...
const AuctionPage = ({ isDarkTheme, userAddress, contractService }) => {
  ...
  const calcReceived = async (day, memberEntry, currentRawAmount, pool) => {
    ...
    let calcTotalReceive =
      +dailyEntryCurrentDay !== 0
        ? new BigNumber(
            currentRawAmount.dividedBy(dailyEntryCurrentDay).multipliedBy(pool),
          )
        : new BigNumber(0);
    ...
    if (
      memberEntry.referrerAddr &&
      memberEntry.referrerAddr !== 0 &&
      memberEntry.referrerAddr.toUpperCase() !== userAddress.toUpperCase() &&
      memberEntry.referrerAddr.toUpperCase() !==
        defaultReferrerAddr.toUpperCase()
    ) {
  ...
  const getRows = React.useCallback(
    async (page = 1, days, auctionObj) => {
      setAuctionsRows([]);
      let newAuctionsRows = [];

      const newPageCount = Math.ceil(
        new BigNumber(days).minus(+auctionObj[1]).dividedBy(5),
      );
      ...
      const endPoint =
        +startPoint - 5 < +auctionObj[1] ? +auctionObj[1] : +startPoint - 5;
      ...
      for (let i = startPoint; i >= endPoint; i--) {
        ...
        auctionRow.pool = new BigNumber(auctionRow.pool)
        ...
        for (let j = 0; j < +memberObj.tailIndex; j++) {
          ...
          currentRawAmount = currentRawAmount.plus(
            new BigNumber(memberEntry.rawAmount).dividedBy(
              new BigNumber(10).pow(decimals.ETH),
            ),
          );
        }

        const received = await calcReceived(
          i,
          memberEntry,
          currentRawAmount,
          auctionRow.pool,
        ...
        auctionRow.received = new BigNumber(received)
          .multipliedBy(new BigNumber(10).pow(decimals.ETH))
          .toFixed();
        ...
        auctionRow.dailyEntry = new BigNumber(dailyEntry)
        ...
        auctionRow.eth =
          +auctionRow.dailyEntry !== 0
            ? new BigNumber(auctionRow.pool)
                .dividedBy(auctionRow.dailyEntry)
                .toFixed()
            : 0;
        ...
        contractService
          ...
          .then(async (res) => {
            ...
            for (let i = res[0] ? +res[1] : days; i <= days; i++) {
              ...
              const memberObj = await contractService.xfLobbyMembers(
                i,
                userAddress,
              );
              let memberEntry = {};
              ...
              for (let j = 0; j < +memberObj.tailIndex; j++) {
                totalLotteryEntryes++;
                memberEntry = await contractService.xfLobbyEntry(
                  userAddress,
                  i,
                  j,
                );
              ...
              if (+currentRawAmount.toFixed() > 0) {
                ...
                let received = await calcReceived(
                  i,
                  memberEntry,
                  currentRawAmount,
                  pool,
                ...
                if (+memberObj.tailIndex > 0) {
                  ...
                  newAverageRate = newAverageRate.plus(
                    new BigNumber(pool).dividedBy(
                      new BigNumber(dailyEntry).dividedBy(
                        new BigNumber(10).pow(decimals.ETH),
                      ),
                    ),
                  );
            ...
            newAverageRate =
              totalLotteryEntryes === 0
                ? 0
                : newAverageRate.dividedBy(totalLotteryEntryes);
