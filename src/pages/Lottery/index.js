...
const LotteryPage = ({ isDarkTheme, userAddress, contractService }) => {
  ...
  const getWinners = React.useCallback(
    async (days) => {
      try {
        ...
        for (let i = days; i >= 0; i--) {
          ...
          if (winner.who !== '0x0000000000000000000000000000000000000000') {
            const winnerObj = {
              day: i,
              date: '20.10.2020',
              amount: new BigNumber(winner.totalAmount)
  ...
  const getMembersPromises = (day) => {
    return new Promise((resolve, _) => {
      contractService.loteryCountLen(day).then((res) => {
        let membersPromises = [];
      ...
      .then((res) => {
        ...
        if (res.who !== '0x0000000000000000000000000000000000000000') {
          ...
          setLotteryWinner({
            ...
            totalAmount: new BigNumber(res.totalAmount)
              .dividedBy(new BigNumber(10).pow(decimals.ETH))
              .toFixed(),
          });
    ...
    contractService
      ...
      .then((days) => {
        ...
        Promise.all([
          ...
          contractService.loteryDayWaitingForWinnerNew(),
        ])
          .then((lotteryDays) => {
            const day =
              +lotteryDays[1] < +days ? lotteryDays[1] : lotteryDays[0];
            contractService.getDayUnixTime(day).then((date) => {
        ...
        getMembersPromises(days - 1).then((result) => {
              ...
              .then((result) => {
                ...
                for (let key in members) {
                  members[key] = members[key]
                    ? ((members[key] * 100) / allChancesCount).toFixed(2)
                    : 0;
                }
                if (!(res % 2)) {
            ...
            {navItems.map((item, index) => {
              return (
                <div
                  ...
                  className={classNames('nav__item p-lottery__nav-item', {
